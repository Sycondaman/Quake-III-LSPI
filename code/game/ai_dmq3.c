	...
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		...
		if (curenemy < 0 && squaredist > Square(100) && !healthdecrease && !EntityIsShooting(&entinfo))
		{
			...
			if (!InFieldOfVision(entinfo.angles, 90, angles)) {
				...
				if(!bs->bottype)
				{
					// Don't make this decision for our bots
					if (BotWantsToRetreat(bs)) continue;
				}
			}
		}
		//found an enemy
		if (curenemy >= 0 || (curenemy == -2 && bs->enemy >= 0))
		{
...
BotDeathmatchAI
==================
This function is called periodically to execute the current action. To execute an action, the function pointer bs->ainodes
is used. The AINodes will then call trap_EA_<Action> which will update the bot input struct.
*/
void BotDeathmatchAI(bot_state_t *bs, float thinktime) {
	...
	if (!bs->ainode) {
		if(!bs->bottype)
		{
			BotAI_Print(PRT_MESSAGE, "No node: Quake Bot.\n");
		}
		else if(bs->bottype == 1)
		{
			BotAI_Print(PRT_MESSAGE, "No node: LSPI Bot.\n");
		}
		else if(bs->bottype = 2)
		{
			BotAI_Print(PRT_MESSAGE, "No node: Gradient Bot.\n");
		}
		else
		{
			BotAI_Print(PRT_ERROR, "No node: Invalid Bot.\n");
		}
