...
BotUpdateInput
==============
Calls trap_EA_GetInput to retrieve the current bot_input_t struct for the given bot. This is updated by the code path from ai_dmq3->BotDeathmatchAI.
Then the bot_state_t->lastucmd is set to whatever is returned by transforming the bot_input_t into a user command which can be sent to the server.
*/
...
void BotResetState(bot_state_t *bs) {
	...
	BotFreeWaypoints(bs->checkpoints);
	BotFreeWaypoints(bs->patrolpoints);
	//reset the whole state EXCEPT for the bottype
	memset(bs, 0, sizeof(bot_state_t) - sizeof(int));
...
BotAIStartFrame
==================
This is called every frame and at the end will update the BotInput with a call to BotUpdateInput.
*/
